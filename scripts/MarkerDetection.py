...
class MarkerDetector:
    ...
    def Aruco_marker_detector(self):
        ...
        if markerCorners:
                ...
                # change form in global frame
                # rotMat1 = tr.euler_matrix(0, 0, self.pos[3])
                # rotMat1 = rotMat[0:3, 0:3]
                # setpose = np.zeros((3,1), dtype=np.float)
                # setpose[0] = -tvecs[1]
                # setpose[1] = -tvecs[0]
                # setpose[2] = -tvecs[2]
                # # setpose = np.matmul(rotMat1, setpose)
                # # the position of the marker in body frame
                # marker_pos = PT()
